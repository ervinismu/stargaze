...
function nodeR(d){ return Math.max(5, Math.min(22, 5 + Math.log2(Math.max(1, d.stars)) * 1.4)); }
...
function showApp(username){
  ...
  const topLangs = Object.entries(graphData.langMap)
    .sort((a,b)=>b[1].length-a[1].length).slice(0,14);
  ...
  topLangs.forEach(([lang, idxs]) => {
    ...
    el.addEventListener('click', () => {
      ...
      document.querySelectorAll('.lang-item').forEach(e => e.classList.toggle('active', e.dataset.lang === filterLang));
      rebuildGraph();
    });
...
function rebuildGraph(){
  ...
  const W = svgEl.clientWidth || window.innerWidth - 185;
  const H = svgEl.clientHeight || window.innerHeight - 42;
  ...
  mkFilter('glow', 3); mkFilter('glow2', 6);
  ...
  const link = g.append('g').selectAll('line').data(simLinks).join('line')
    .attr('stroke','#1e2d40').attr('stroke-width',0.9).attr('stroke-opacity',0.75);

  const node = g.append('g').selectAll('circle').data(simNodes).join('circle')
    .attr('r', nodeR)
    .attr('fill', d=>d.color).attr('fill-opacity',0.88)
    .attr('stroke', d=>d.color).attr('stroke-width',0.5).attr('stroke-opacity',0.4)
    ...
    .on('mouseover', function(e,d){
      d3.select(this).attr('r', nodeR(d)+4).attr('fill-opacity',1).attr('filter','url(#glow2)');
      showTooltip(e, d);
    })
    .on('mousemove', e => moveTooltip(e))
    .on('mouseout', function(e,d){
      d3.select(this).attr('r', nodeR(d)).attr('fill-opacity',0.88).attr('filter','url(#glow)');
      hideTooltip();
    })
    .on('click', function(e,d){
      e.stopPropagation();
      const orig = allNodes.find(n=>n.id===d.id)||d;
      openDetail(orig);
      hideTooltip();
    });
...
document.getElementById('btn-reset').addEventListener('click', () => {
  if(sim) sim.stop();
  allRepos=[]; graphData=null; filterLang=null; searchVal=''; selectedNode=null;
